    ...
    public static class TracingUntil
    {
        ...
        public static List<TracingSpan> BuildTracingSpan(this RepeatedField<Span> spans)
        {
            ...
            foreach (var item in spans)
            {
                TracingSpan tracingSpan = new TracingSpan()
                {
                    ...
                    Flags = item.Flags,
                    StartTime = item.StartTime.ToDateTime(),
                    Duration = item.Duration.ToTimeSpan()
                };
        ...
        public static List<SpanTag> BuildTags(this RepeatedField<KeyValue> spans)
        {
            ...
            foreach (var item in spans)
            {
                ...
                switch (item.VType)
                {
                    case Jaeger.ApiV2.ValueType.String: tag.VStr = item.VStr; break;
                    case Jaeger.ApiV2.ValueType.Bool: tag.VBool = item.VBool; break;
                    case Jaeger.ApiV2.ValueType.Int64: tag.VFloat64 = item.VInt64; break;
                    case Jaeger.ApiV2.ValueType.Float64: tag.VFloat64 = item.VFloat64; break;
                    case Jaeger.ApiV2.ValueType.Binary: tag.VBinary = ByteStringToString(item.VBinary); break;
                }
                tags.Add(tag);
            }
